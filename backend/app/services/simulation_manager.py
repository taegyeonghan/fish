...
from .local_graph import LocalEntityReader, FilteredEntities
from .oasis_profile_generator import OasisProfileGenerator, OasisAgentProfile
from .simulation_config_generator import SimulationConfigGenerator, SimulationParameters
from ..utils.locale import t

logger = get_logger('ungdroofish.simulation')
...
class SimulationManager:
    ...
    def _get_existing_simulation_dir(self, simulation_id: str) -> Optional[str]:
        """Return an existing simulation directory without creating it."""
        base_dir = os.path.abspath(self.SIMULATION_DATA_DIR)
        sim_dir = os.path.abspath(os.path.join(base_dir, simulation_id))

        if sim_dir != base_dir and not sim_dir.startswith(base_dir + os.sep):
    ...
    def delete_simulation(self, simulation_id: str) -> bool:
        """Delete a simulation directory and remove it from the in-memory cache."""
        sim_dir = self._get_existing_simulation_dir(simulation_id)
        if not sim_dir:
            return False
    ...
    def get_run_instructions(self, simulation_id: str) -> Dict[str, str]:
        ...
        return {
            ...
            "instructions": (
                f"1. conda: conda activate UngdrooFish\n"
                f"2.  ( {scripts_dir}):\n"

...
설계 원칙:
1) 아래 "권장 기본 타입"(투자 도메인 표준 엔티티)을 우선 재사용하고, 문서에서 명확히 근거가 있는 경우에만 도메인 특화 타입을 추가합니다.
...
절대 금지 — 다음과 같은 "추상/메타 타입"은 만들지 마세요 (실제 세계의 대상이 아니라 분석 과업 자체를 가리키므로 무의미한 노드를 양산합니다):
- 분석/예측 과업, 방법론, 모델, 전략을 가리키는 타입 (예: PredictionModel, ForecastModel, InvestmentStrategy, AnalysisTask, Methodology, Scenario).
- 시뮬레이터/문서의 메타데이터를 가리키는 타입 (예: Question, Document, Source, Report).
엔티티 타입은 반드시 현실에 실재하는 대상(종목·기업·인물·섹터·지표·이벤트·기관·시장 등)만을 분류해야 합니다.

권장 기본 타입 (가능하면 이 이름 그대로 재사용):
Stock, Investor, Market, Sector, EconomicIndicator, Event, FinancialMetric, Organization, Person
"""
...
"""
    ...
    def _validate_and_process(self, result: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # 메타/추상 타입(Model·Report·Prediction 등) 제거 — 현실의 대상만 남긴다
        try:
            blocked_re = re.compile(Config.BLOCKED_ENTITY_PATTERN)
        except re.error:
            blocked_re = None
        if blocked_re is not None:
            kept = []
            removed_names = set()
            for entity in result["entity_types"]:
                name = entity.get("name", "")
                if name and blocked_re.search(name):
                    removed_names.add(name)
                    logger.warning(f"Meta/abstract entity type '{name}' removed during validation")
                    continue
                kept.append(entity)
            result["entity_types"] = kept
            # 제거된 타입을 참조하는 edge source_targets 정리
            for edge in result.get("edge_types", []):
                edge["source_targets"] = [
                    st for st in edge.get("source_targets", [])
                    if st.get("source") not in removed_names and st.get("target") not in removed_names
                ]

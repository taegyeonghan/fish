...
from .local_graph import EntityNode, LocalGraphStore, LocalEntityReader

logger = get_logger('ungdroofish.oasis_profile')
...
class OasisAgentProfile:
    ...
    def to_reddit_format(self) -> Dict[str, Any]:
        ...
        profile = {
            ...
            "username": self.user_name,  # OASIS  username(설명 생략)
            "name": self.name,
            "bio": self.bio,
    ...
    def _search_zep_for_entity(self, entity: EntityNode) -> Dict[str, Any]:
        ...
        try:
            ...
            for sr in search_results:
                if sr.result_type == "edge" and sr.edge and sr.edge.fact:
                    all_facts.add(sr.edge.fact)
                elif sr.result_type == "node" and sr.node:
                    if sr.node.summary:
                        all_summaries.add(sr.node.summary)
                    if sr.node.name and sr.node.name != entity_name:
                        all_summaries.add(f": {sr.node.name}")
            ...
            logger.warning(f" ({entity_name}): {e}")
